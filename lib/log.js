...
function numberLog(data) {
  ...
  const customNumText = text.replace(number, `${chalk.italic.cyan(number)}`);
  const customYearText = text.replace(year, `${chalk.italic.cyan(year)}`);
...
function customNumberLog(text, num) {
  const customNumText = text.replace(num, `${chalk.italic.cyan(num)}`);
  ...
  if (isDate()) {
    ...
    const findDay = searchReplace(text, day, `${chalk.italic.cyan(day)}`);
    const findMonth = searchReplace(
      findDay,
      month,
      `${chalk.italic.cyan(month)}`
    );
